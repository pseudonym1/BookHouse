...
$.extend($.fn.tabs.methods, {
    ...
    loadTabIframe: function (jq, params) {
        return jq.each(function () {
            ...
            $tabBody.html('');
            var mast = window.mast || {};
            var loadImgUrl = mast ? (mast.basePath + "/images/loading.gif") : "../images/loading.gif";

            $tabBody.css({'overflow': 'hidden', 'position': 'relative'});
            var $mask = $('<div style="position:absolute;z-index:2;width:100%;height:100%;background:#ccc;z-index:1000;' +
                'opacity:0.3;filter:alpha(opacity=30);"><div>').appendTo($tabBody);
            var $maskMessage = $('<div class="mask-message" style="z-index:3;width:auto;height:16px;line-height:16px;' +
                'position:absolute;top:50%;left:50%;margin-top:-20px;margin-left:-92px;border:2px solid #d4d4d4;' +
                'padding: 12px 5px 10px 30px;background: #ffffff url(' + loadImgUrl + ') no-repeat scroll 5px center;">' +
                (params.iframe.message || 'Processing, please wait ...') + '</div>').appendTo($tabBody);
